...
(function() {
  ...
  function joinForm(stem, ending) {
    if (!stem) return "—";
    if (ending === null || ending === undefined) return "—";
    let e = String(ending).trim();
    if (!e || e === "—") return "—";

    // alternations like "i / is"
    if (e.includes(" / ")) {
      const parts = e.split(" / ").map(x => x.trim());
      const out = parts.map(p => (p === "∅" ? stem : stem + p));
      return out.join(" / ");
    }
  ...
  function renderParadigmTable(entry, pObj) {
    ...
    let html = "";
    html += '<table class="table lex-paradigm-table">';
    html += "<thead><tr><th>Case</th><th>Singular</th><th>Dual</th><th>Plural</th></tr></thead>";
    html += "<tbody>";
